          ...
          + (persona.type === "CAPYBARA_SWIM"
            ...
            : hasSimpleTubeWater
              ? `<ellipse class="profile-tube-ripple" cx="${geometry.cx}" cy="${geometry.cy - 0.1}" `
                + `rx="${(scaledShadowRx + 1.2 * scale).toFixed(2)}" ry="${(1.75 * Math.sqrt(scale)).toFixed(2)}" `
                + `fill="none" stroke="#79C0FF" stroke-width=".45" opacity=".28"/>`
                + `<path class="profile-tube-wake" d="M${(geometry.cx - scaledShadowRx - 1.4).toFixed(2)} ${(geometry.cy - 0.65).toFixed(2)} `
