...
for (const persona of visible.filter(isBeePersona)) {
  ...
  assert(lightBee.duration >= 84 && lightBee.duration <= 100 && darkBee.duration >= 84 && darkBee.duration <= 100,
    `Bee Quokka ${id} should loop in about 90s for a lively but readable flight.`);
  assert(averageSpeed >= 4,
    `Bee Quokka ${id} should keep a broad full-farm flight; average ${averageSpeed.toFixed(2)} units/s.`);
  assert(maximumSpeed <= 30,
